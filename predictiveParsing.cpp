...
void charArrayCopy(map<char, char[10][10]> &table, int i, int j)
{
    for (int index = 0; index < 10; index++)
    {
        table[nt[i]][j][index] = prod[i][index];
    }
}

bool checkTerminalInFirstSet(int i, int j)
{
    for (int index = 0; index < 10; index++)
    {
        if (ter[j] == first[i][index])
            return true;
    }
    return false;
}

void createTable(map<char, char[10][10]> &table)
{
    for (int i = 0; i < strlen(nt); i++)
    {
        for (int j = 0; j < strlen(ter); j++)
        {
            if (checkTerminalInFirstSet(i, j))
            {
                charArrayCopy(table, i, j);
            }
        }
    }
}
